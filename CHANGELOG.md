...
- **Первый /whois доставляет MX/SSL/DNS по готовности** (TASK-0076). Фоновые проверки (check_ssl/dns/email_intel) теперь досылают обновлённые блоки в чат (follow-up сообщения) вместо ожидания повторного запроса.
- **Глубокий e-mail больше не возвращает пусто** (TASK-0077). В `fetch_deep_email` настроены публичные DNS-резолверы (1.1.1.1/8.8.8.8) для контейнера; добавлены диагностические логи; интеграционный тест на google.com.

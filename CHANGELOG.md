...
- **DNS-отчёт** (TASK-0090, ADR 044): кнопка «🧾 DNS-отчёт» в карточке `/whois` — профессиональный анализ всех DNS-записей домена (SOA, NS, A, AAAA, CNAME, MX, TXT, SRV, CAA, обратные PTR, DNSKEY/DS), признак DNSSEC и security-проба AXFR (открытый трансфер зоны), сгруппированных по типам и выгруженных текстовым файлом. Без БД-кэша; при сбое — сообщение об ошибке (TASK-0086).
...
- **Webhook: хостовый publish-порт вынесен в `WEBHOOK_HOST_PORT` (дефолт 8091)** (hotfix, 2026-07-11). Инцидент: на хосте-поллере общеизвестный 8080 занял осиротевший чужой процесс — `ww-bot` не мог опубликовать docker-порт и висел в краш-цикле. Внутренний бинд бота и healthcheck контейнера остаются на 8080; наружу публикуется `127.0.0.1:${WEBHOOK_HOST_PORT}:8080`. Подтянуты потребители порта: `scripts/deploy.sh` (пост-деплойный health-check читает реальный published-порт через `docker compose port`, а не хардкод 8080) и nginx-upstream в `docs/deployment.md`. Класс бага anti-drift: значение поменяли — потребители отстали.
- **Email-intel: upsert писал в несуществующее поле `last_checked_at`** (follow-up к TASK-0089, найден исполнителем в TASK-0095): у `EmailIntelCache` поле называется `fetched_at` — успешная проверка email-слоя падала на записи в кэш. Класс бага TASK-0017 (anti-drift).
- **«Свободен» больше никогда не отдаётся из кэша бота** (TASK-0094, ADR 046; повтор класса инцидента 0091: свободный домен проверили → зарегистрировали → бот продолжал отвечать «свободен»). Если запись в `whois_cache` описывает незарегистрированный домен, `/whois` и карточки всегда делают live-запрос (с RDAP-кросс-чеком ADR 045) вместо отдачи кэша «свежестью» до 6 часов; при сбое live — честная ошибка, а не stale-«свободен». Кеш негативных ответов на стороне whoisd-прокси/relay убирается отдельно (TASK-0095, infra: TTL 0 + микро-дедуп 60с, включая ветку RDAP-404).
- **«Свободен» только после верификации** (TASK-0092, ADR 045; инцидент TASK-0091 — зарегистрированный домен 2 суток показывался свободным из-за stale «No entries» от relay/TCI). Ответ «свободен» из WHOIS-текста теперь перепроверяется независимым RDAP-запросом: RDAP видит регистрацию → бот показывает «занят» с данными RDAP; RDAP недоступен → честное «похоже свободен, подтвердить не удалось» вместо уверенного «свободен». Тексты ошибок/рейтлимитов/HTML от upstream больше не трактуются как «домен свободен».
- **Email-intel/deep: MX и почтовые отчёты пустые для доменов с TXT-записями** (TASK-0089, корень найден прод-диагностикой TASK-0088). Email-слой звал несуществующий `TXT.to_unicode()` (7 мест) — AttributeError ронял весь `fetch_email_intel` (включая уже полученный MX) и `fetch_deep_email`. Теперь `txt_to_str()` через официальный `rdata.strings` (корректная склейка многосегментных SPF, RFC 7208 §3.3). Тесты переведены с моков несуществующего метода на реальные dnspython rdata.
...
(ADR 040). Корень бага «не показывается MX / глубокий e-mail пуст».

### Fixed

- **DNS-сбой ≠ «нет записей».** `fetch_email_intel` раньше любой не-NXDOMAIN сбой резолва MX (timeout / no-nameservers / резолвер не сконфигурирован) трактовал как «MX нет» → кэш писался `is_reachable=True, mx=[]`, и карточка `/whois` показывала ложное «MX: не настроен» для домена, у которого MX есть. Теперь такой сбой → состояние «не отвечает» (`dns_unreachable`), а не ложное пусто (TASK-0079).
- Аналогичная диагностика в глубоком e-mail (`deep_client`): сбой резолва логируется, а не молча даёт пустой раздел.

### Added

- **Конфигурируемые DNS-резолверы** через `DNS_NAMESERVERS` (список через запятую; пусто = системный resolver, поведение по умолчанию). Ops-рычаг на случай, когда системный DNS в контейнере воркера ограничен.
- Общая фабрика `build_resolver` + классификатор `classify_dns_exc` (`src/email_intel/resolver.py`); диагностические warning-логи при сбоях резолва.

### Internal

- Таски 0079/0080 (hotfix-стек v0.15.2). Тесты: классификация DNS-ошибок, `build_resolver`, регресс MX (timeout→unreachable, NoAnswer→пустой MX) + opt-in интеграц-тест на `pinspb.ru`.

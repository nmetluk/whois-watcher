...
- **Идентификатор инстанса в сообщениях админ-канала (ADR 019).** Каждое
  сообщение в админ-канал теперь начинается с тега `[label · domain · ip]`,
  собранного из конфига (`INSTANCE_NAME`, домен из `WEBHOOK_BASE_URL`,
  `SERVER_IP`). При развёртывании на нескольких машинах сразу видно, с какого
  сервера пришёл алерт. Пустые части тега опускаются. Новые env-переменные:
  `INSTANCE_NAME`, `SERVER_IP` (TASK-0019).

### Changed

- Конвенция логирования (`CLAUDE.md`): runtime IP сервера по-прежнему НЕ
  логируется в structlog, но допускается в приватном админ-канале (ADR 019)
  через явный конфиг `SERVER_IP`.

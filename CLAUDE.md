...
- **Хотя бы один рендер-тест форматтера — через настоящий `t()` (без мока).**
  Если все тесты форматтера мокают `t()` (`side_effect=lambda key,…: f"[{key}]"`),
  реальный `template.format(**kwargs)` не выполняется, и рассинхрон шаблона и
  аргументов проходит незаметно. Урок TASK-0046: `format_email_deep` падал
  `KeyError: 'exceeds'` (шаблон `"lookups: {count}{exceeds}"` без передачи
  `exceeds`), а мок-`t()`-тесты были зелёными. Правило: на каждый форматтер —
  минимум один тест с реальным `t()`, прогоняющий ветки с placeholder'ами.
...
- ❌ **Не деплоить/накатывать миграции без свежего ручного бекапа БД.**
  Перед любым деплоем — `pg_dump` + проверка непустоты (см.
  `docs/deployment.md` → «ОБЯЗАТЕЛЬНО перед каждым деплоем»). Урок инцидента:
  переименование тома → пустая БД, данные не восстановили.
- ❌ Не запускать `docker compose down -v` / `docker volume rm` /
  `docker volume prune` на проде — сотрёт том БД.

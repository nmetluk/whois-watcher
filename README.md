...
Для уже развёрнутого бота — одна команда:

```bash
bash scripts/deploy.sh
```

Скрипт автоматически: проверит чистоту дерева, подтянет последние коммиты,
пересоберёт образы, накатит миграции, перезапустит сервисы и убедится, что
всё запустилось. Подробности — в [docs/deployment.md](docs/deployment.md).

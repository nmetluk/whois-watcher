...
## ⚠️ ОБЯЗАТЕЛЬНО перед каждым деплоем/апдейтом — ручной бекап БД

**Требование (не рекомендация).** Перед любым деплоем, накатом миграций или
изменением `docker-compose.yml`/томов — снять ручной дамп Postgres и убедиться,
что он непустой. Это правило введено после инцидента: при апдейте Postgres
поднялся на новом пустом томе (переименование тома), а автоматических бекапов
ещё не было — данные восстановить не удалось (тогда — тестовые аккаунты).

```bash
# 1. Ручной дамп ПЕРЕД деплоем (custom-формат)
docker exec ww-postgres pg_dump -U whoiswatcher -d whoiswatcher -Fc \
  > "predeploy-$(date -u +%Y%m%dT%H%M%SZ).dump"

# 2. Проверить, что дамп валиден и непустой
ls -lh predeploy-*.dump            # размер заметно больше нуля
pg_restore --list predeploy-*.dump | head   # без ошибок = дамп читаем

# 3. Только теперь — деплой/миграции.
```

Чек-лист перед деплоем:

- [ ] Снят ручной `pg_dump`, файл непустой, `pg_restore --list` без ошибок.
- [ ] Не менялось имя тома Postgres (`ww_postgres_data`) и `PGDATA`
      (`/var/lib/postgresql/data/pgdata`) — иначе БД стартует пустой.
- [ ] Если меняли `docker-compose.yml` — проверить, что `postgres` всё ещё
      монтирует тот же том (`docker compose config | grep -A2 volumes`).
- [ ] **Никогда** не запускать `docker compose down -v` / `docker volume rm` /
      `docker volume prune` на проде.

> С v0.15 есть и автоматический ежечасный бекап (ADR 042, `backup_postgres`),
> но ручной дамп прямо перед деплоем — отдельное обязательное звено: он снимает
> состояние ровно до изменения и не зависит от того, успел ли отработать cron.

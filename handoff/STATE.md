...
**Обновлено:** 2026-06-02 (v0.13 фич-код влит и протестирован; аудит v0.13 проведён — вердикт fix-then-go, блокер 🟠 TASK-0047 SSRF MTA-STS) · **Релиз на main:** v0.12.0 · **Последний ADR:** 040
...
| Deep email + on-demand views (MX инлайн, кнопки) | v0.13 | 040 ✅ | дизайн готов; исполнение TASK-0038…0044 |
...
вывез (затык в моке aiohttp: `session.get` мокали `AsyncMock` → корутина в
`async with`); архитектор применил корректный тест сам (`session.get` —
синхронный `MagicMock`, отдающий async-CM) и смержил. ⚠️ Локальный `pytest` в
песочнице не гонялся (нет Python 3.11) — финально подтвердить зелёный CI.

**Следующий шаг:** **TASK-0044 «Релиз v0.13.0»** разблокирован (0043/0047 done):
bump `pyproject` 0.12.0→0.13.0, секция CHANGELOG `[0.13.0]`, тег, деплой. После —
🟡/🟢 **TASK-0048** (cleanup, v0.13.1). Дальше по roadmap — **v1.0** (web-дашборд,

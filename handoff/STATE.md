# STATE — живой снимок состояния проекта

> Носитель контекста между сессиями. Любой агент читает это вторым
> (после `handoff/README.md`), чтобы понять «где мы сейчас». Обновляется:
> архитектором — после merge крупных кусков; исполнителем — раздел
> «Последняя сессия». Дата последнего обновления — обязательна.

**Обновлено:** 2026-05-29 · **Релиз на main:** v0.8.1 · **Последний ADR:** 035

## Где мы сейчас

Бот в проде, четыре оси наблюдения работают: WHOIS, SSL, DNS (A/AAAA/NS),
RIR/ASN-фундамент. Введён новый рабочий процесс (handoff + PR), заменивший
ручной промпт + Telegram + монолитный SESSION_LOG.

## Активный план

Источник: `PLAN_subdomains_wishlist.md` в корне репо.

| Этап | Релиз | ADR | Состояние |
|------|-------|-----|-----------|
| Багфикс wishlist ↔ tracked | v0.8.1 | 034 ✅ | TASK-0001 done, **релиз v0.8.1 выпущен** |
| Поддомены / PSL / DNS-SSL у поддомена | v0.9.0 | 035 ✅ | TASK-0002…0005 done (merged); код v0.9.0 готов; аудит 0006 → релиз |
| Domain intelligence (MX/SPF/DKIM/DMARC, subdomain enum) | v0.10+ | 036 | дизайн |

ADR 034 и 035 дописаны в `docs/decisions.md`. Цепочка зависимостей
v0.9.0: 0002→0003→0004→0005→0006 (аудит); все depend на TASK-0001.

## Ключевые активные решения

- Wishlist-фикс: `/add` на wishlist-домен **тихо промоутит** в tracked;
  разделы `/list` и `/wishlist` остаются раздельными.
- Поддомены: WHOIS берётся у registrable-родителя (eTLD+1 через
  **tldextract**), DNS/SSL — у самого поддомена. PSL в **оффлайн-режиме**
  с bundled snapshot (без сети в горячем пути), `include_psl_private_domains=False`.
- Процесс: GitHub = источник правды; исполнитель пишет только в git;
  один таск = ветка `task/NNNN-slug` = PR; merge — архитектор.

## Следующий шаг

Все четыре подэтапа v0.9.0 (TASK-0002…0005) смержены: PSL, схема,
WHOIS-роутинг, UX поддоменов + заполнение `registrable_domain`/
`is_subdomain` при вставке. `pyproject` на 0.9.0. Следующий и
последний перед релизом — **`TASK-0006`** (комплексный аудит v0.9.0,
отдельная сессия). После аудита и закрытия findings — тег v0.9.0.

При мерже TASK-0005 архитектором добавлены 2 правки: guard на
`is_public_suffix_only` в `cmd_whois`/`cmd_add` (мусорный ввод ронял
хэндлер) + опечатка в docstring formatters. Учесть в аудите.

⚠️ Долг к закрытию до релиза v0.9.0 (вынести в аудит TASK-0006):
миграция `20260529_registrable_domain` оставляет `server_default=""` на
`registrable_domain`, а модель дефолта не имеет — рассинхрон. После
того как TASK-0005 начнёт заполнять поле при вставке, server_default
надо снять (отдельной миграцией или поправкой), чтобы БД не плодила
пустые registrable.

## Последняя сессия

_(заполняет исполнитель: ссылка на docs/sessions/SESSION-*.md, что сделано,
что осталось, открытые вопросы)_

— пока нет.

## Открытые вопросы

Закрыты дефолтами в ADR 035: PSL — оффлайн bundled snapshot без сетевого
автофетча; `include_psl_private_domains=False`; поддомены в `/list` со
значком `↳`. Новых открытых вопросов нет.

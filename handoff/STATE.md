...
**Обновлено:** 2026-06-06 (✅ TASK-0096: whois-proxy `34ec1b8`+`5d19325` — хостовые патчи 0095 версионированы, хосты перекатаны из git, **бот редеплоен** (`1957328`, бекап manual-pre-0096) — ADR 046 полностью в проде: «свободен» нигде не живёт в кэше >60с, инцидент discozavr.ru невоспроизводим. Хвост: real-world e2e владельцем (/version + регистрация). ⚠️ аудиту: `BIND_ADDR_HTTP` дефолт теперь 0.0.0.0 (ufw deny-by-default) — проверить закрытость 8043 снаружи. Процессное: исполнитель 2-й раз не пушит отчёт — отчёты 0095/0096 доводил архитектор) · **Релиз на main:** v0.16.0 + hotfix'ы + 0085-0096 (прод = `1957328`) · **Последний ADR:** 046
...
✅ **0081/0082/0083 смержены — все блокеры аудита 0071 закрыты** (2026-06-11, PR
#55/#56/#57, ревью архитектора). **0081** (F1): `/bulk` (delete/add_to_group,
scoped), `/import` (реальный через `csv_io`+`DomainService`, счётчики
imported/invalid/truncated), `/alerts/read` — честный ack (read-флага в модели
нет, задокументировано); попутно фикс латентного no-op `remove_wishlist`.
**0082** (F2): убран demo-fallback во всех экранах → error-state с retry + честный
empty; «Прочитать все» теперь зовёт `markAlertsRead`. **0083** (F3–F7): initData
TTL 24ч→1ч, dev-initData-в-URL гейтнут за `environment=development`, `OPTIONS`
пропускается до auth (preflight ок), raw `sa_delete` → `DomainRepository.remove_by_id`,
CSP в nginx + docs. Архитектор прогнал локально webapp_auth + groups + dns-classify
тесты (зелёные); api.py-оверлап 0081↔0083 слит чисто.

✅ **РЕЛИЗ v0.16.0 выпущен** (2026-06-11, TASK-0072, архитектором): bump
0.15.2→0.16.0, CHANGELOG `[0.16.0]`, тег `v0.16.0`. Telegram WebApp (ADR 043):
дашборд/список/карточка/календарь/алерты/«Ещё», backend `/api/webapp` read+write
+initData-auth, группы/теги, CSV-импорт. Аудит 0071 (F1–F10) закрыт; фиксы
0081–0084 влиты. 0084 также ресинкнул uv.lock (matplotlib drift).

✅ **v0.16.0 задеплоен в прод** (2026-06-11, владельцем). Telegram WebApp живой:
backend `/api/webapp` + mini-app (дашборд/список/карточка/календарь/алерты/«Ещё»),
группы/теги, CSV-импорт. v0.15.2 задеплоен ранее.

**Следующий шаг:** **v1.0** — web-дашборд вне Telegram, публичный API,
орг-аккаунты, Prometheus. Перед стартом — спроектировать ADR (новый этап).
Опционально: следить за прод-фидбеком по webapp (как было с email/MX —
реальные UX-баги ловятся только в Telegram).
...
**2026-06-07 (исполнитель + архитектор) — TASK-0093: relay/whoisd — короткий TTL негативных ответов**

Закрыт первоисточник инцидента discozavr.ru. Корень: whoisd (proxy на
прод-хосте и edge на VDS) кэшировал ответ TCI по принципу «useful vs error»,
а не «registered vs free» — легитимный «No entries» (len>50, не рейтлимит)
считался полезным и жил 24ч. Плюс лаг самого TCI: «No entries» пришёл за
~13 мин ДО фактической регистрации (22:28 vs 22:41), и stale-версия
застряла на сутки. Фикс (на хостах, в git — только отчёт): `NO_DATA_RE`
в whoisd → негативные ответы кэшируются ≤1ч (`*_TTL_NEG=3600`), позитивные
по-прежнему 24ч; `ok=true` для «No entries» сохранён, чтобы бот видел текст
и 0092 мог отличать free от ошибки. В связке с 0092 (RDAP-verify) симптом
...
**2026-06-07 (архитектор, №7) — TASK-0092: RDAP-верификация «свободен» (ADR 045)**

По уликам отчёта 0091 (легитимный stale «No entries» от relay/TCI):
в `lookup_domain` добавлен `_verify_unregistered` — «свободен» из
WHOIS-текста перепроверяется независимым RDAP (positive beats negative);
RDAP registered → показываем ЗАНЯТ; RDAP недоступен → честное
«подтвердить не удалось» (`free_unverified`). Плюс guard «сбой ≠ свободен»
(`looks_like_upstream_error`: рейтлимит/HTML/502 → WhoisError) в
proxy_client и direct-пути. Без миграций (флаги в raw_data). 16 новых
тестов (вкл. реальный текст TCI из улик 0091), 267 смежных зелёные.
Открыт TASK-0093 (инфра): relay/VDS — негативные ответы кэшировать коротко.
Отчёт: `docs/sessions/2026-06-07_task-0092-free-verification.md`.

---


**2026-06-07 (executor) — TASK-0091: Прод-диагностика — зарегистрированный домен 2 суток «свободен» (discozavr.ru)**

Симптом: discozavr.ru (зарег. ~2д назад) → бот показывает «свободен» (даже после истечения кэшей).

Улики:
- whois_cache: 0 rows (домен не tracked, не orphan'ится в persistent).
- Proxy (whois_ru_upstream, cached): `No entries found for the selected source(s).\n\nLast updated on 2026-06-04T22:28:01Z` (ok:true, но текст negative).
- Прямой whois -h tcinet с хоста: network unreachable (43/tcp ограничен; прокси имеет доступ).
- Парсер (в worker): `_looks_like_not_found` → True на "no entries found" → is_registered=False (точно воспроизведено).
- Сравнение: yandex.ru → state: REGISTERED + ns.

Вывод: «свободен» родился в **RU-relay (TCI)** (вернул no-entries) + **parser** (широкий substring NOT_FOUND_PATTERNS по всему тексту, без различения "ошибка upstream" vs "реально free"). Совпадает с гипотезой таска. on-demand пути (не только scheduler) затронуты.

Без код-изменений. Рекомендация: TASK-0092 (ужесточить free-детекцию per-zone, "сбой ≠ свободен", positive state: REGISTERED для .ru, тесты).

Отчёт: `docs/sessions/2026-06-07_task-0091-prod-diagnose-stale-free.md`.

---

**2026-06-05 (архитектор, №6) — TASK-0090: DNS-отчёт (ADR 044)**

Новая on-demand фича по запросу владельца: кнопка «🧾 DNS-отчёт» в карточке
/whois выгружает все DNS-записи домена (SOA/NS/A/AAAA/CNAME/MX/TXT/SRV/CAA +
PTR + DNSKEY/DS + AXFR-проба) текстовым файлом, сгруппированными по типам.
Новая подсистема `src/dns_report/` (client/formatter/types) + ARQ-задача
`check_dns_report` (без БД-кэша, redis-guard, доставка .txt, фейл→0086).
TXT через `txt_to_str` (урок 0089). 11+2 теста. Milestone v0.17.0.
Отчёт: `docs/sessions/2026-06-05_task-0090-dns-report.md`.

---

**2026-06-05 (архитектор, №5) — TASK-0089: фикс TXT.to_unicode() по результатам прод-диагностики 0088**

Отчёт исполнителя по TASK-0088 (качественный, DoD выполнен) опроверг
гипотезы «несвежий worker»/«egress» и дал точный `last_error`:
`'TXT' object has no attribute 'to_unicode'`. У TXT-rdata dnspython
такого метода никогда не было (есть у Name) — 7 вызовов в
client.py/deep_client.py роняли email-intel и deep для любого домена
С TXT-записями (домены без TXT проходили — отсюда «то работает, то нет»).
Тесты были зелёными из-за моков несуществующего метода — **четвёртый
anti-drift-инцидент**. Фикс: `txt.py::txt_to_str` (rdata.strings,
RFC 7208 §3.3), тесты на реальных rdata, моки заменены. 171 email-тест
зелёный. Отчёт: `docs/sessions/2026-06-05_task-0089-fix-txt-rdata-to-unicode.md`.
Хвост из 0088: crt.sh 502 transient — кандидат на retry-таск.

---

**2026-06-05 (архитектор, №4) — TASK-0086: on-demand задачи сообщают об ошибке + диагностика прод-инцидента**

Жалоба «поддомены не присылаются, MX пуст, deep email нулевой»: код
доставки/email-слоя идентичен рабочему v0.15.2 (diff пуст) — корень на
проде (несвежий worker-образ или сломанный egress из контейнеров),
чек-лист для админа — `docs/sessions/2026-06-05_diagnosis-ondemand-email-prod.md`.
Системный пробел закрыт TASK-0086: `deliver_ondemand_failure` вшит в
fail-paths всех 5 задач с `deliver_chat_id` — при фейле пользователь
получает «⚠️ Не удалось…», а не тишину. 17 новых тестов (реальный `t()`,
spec-моки). Отчёт: `docs/sessions/2026-06-05_task-0086-ondemand-failure-delivery.md`.
...
Real-world тест v0.16.0 в Telegram провалился: интерфейс «совершенно не
соответствует дизайну». Диагноз: исполнитель не перенёс CSS дизайна
(`design/webapp/v1/app/`, ~53 КБ, 194 класса), а написал свой с нуля
(~6,7 КБ) — 42 из 55 используемых классов не имели правил, 5 переменных
(`--pv-fg-body` и др.) не были определены; `telegram-web-app.js` не был
подключён в `index.html` (нет темы/BackButton/MainButton/initData);
шрифты шли с Google Fonts, который режется нашим же CSP (F7) — иконки
рендерились голым текстом; `npm run build` (tsc) вообще не проходил.
Фикс: дизайн-CSS портирован вербатим (tokens/ds/tg-chrome, минус
.tg-viewport/.tg-phone), шрифты самохостятся (@fontsource/pt-sans +
material-symbols), скрипт Telegram подключён, разметка ListScreen/DomainRow
приведена к канону дизайна, мёртвые Vite-файлы удалены. Верификация:
0 классов/0 переменных без определения, build зелёный.
Отчёт: `docs/sessions/2026-06-05_hotfix-webapp-design-fidelity.md`.

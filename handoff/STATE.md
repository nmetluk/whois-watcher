...
**v0.16 отдан исполнителю** (параллельные старты): **0066** (backend auth+API),
**0067** (frontend foundation), **0073** (схема групп) — независимы, можно
параллельно. Затем **0068/0069** (экраны; группировка ждёт 0073) → **0070**
(write-действия). Аудит **0071** и релиз **0072** — за архитектором.

**Следующий шаг:** **деплой v0.15.0** (⚠️ образ + `postgresql-client-16`, том
`ww_backups`, env) + опц. TASK-0065; параллельно исполнитель ведёт v0.16.
Дальше — v1.0 (web-дашборд,

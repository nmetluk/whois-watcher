...
**Обновлено:** 2026-06-06 (✅ TASK-0095 смержен: whoisd/relay на обоих хостах — негативы ≤60с (дедуп), RDAP-404 → no-data, юниты почищены; инварианты верифицированы curl'ом. Попутно принят follow-up к 0089: email-upsert писал в несуществующее `last_checked_at` → `fetched_at`. ⚠️ DoD-пробел 0095: whois-proxy репо НЕ обновлён (хосты снова дрейфуют от git) → открыт 🟡 TASK-0096 (upstream патчей). ADR 046 (0094+0095) полностью в силе: «свободен» нигде не живёт в кэше >60с) · **Релиз на main:** v0.16.0 + hotfix'ы + 0085-0095 · **Последний ADR:** 046

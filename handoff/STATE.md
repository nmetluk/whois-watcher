...
**Обновлено:** 2026-06-09 (🔴 хотфикс v0.15.1 по прод-фидбеку: заведены TASK-0075/0076/0077/0078 — доставка on-demand/карточки + пустой deep-email) · **Релиз на main:** v0.15.0 · **Последний ADR:** 043
...
«Глубокий e-mail»; (3) в первом `/whois` после DNS не появляется MX-блок с
подробностями; (4) глубокий e-mail всегда пуст.

**Диагноз (по коду):** #1/#2/#3 — общий дефект: фоновая on-demand работа (ARQ)
наполняет кэш, но **результат не доставляется пользователю** — нет «доставки по
готовности», юзер видит данные лишь при повторном запросе/нажатии (кэш стал
свежим). `check_subdomains` шлёт только diff-уведомления, `check_email_deep`
пользователю не шлёт ничего, карточка `/whois` одноразовая (плейсхолдер не
обновляется). #4 — отдельно: deep-коллекторы (`fetch_deep_email`) возвращают
пусто (подозрение — DNS-resolver в контейнере воркера / передача домена; нужен
интеграц-тест на «богатом» домене типа google.com).

Заведены: **TASK-0075** (досыл результата on-demand кнопок) → **0076** (доставка
MX/SSL/DNS в карточке `/whois` по готовности) → **0077** (диагностика+фикс пустого
deep-email + интеграц-тест) → **0078** (релиз v0.15.1). 0075/0076/0077
независимы; 0078 depends на всех. Хотфикс идёт **впереди** webapp-консолидации
(прод-баги > новая фича).

**Следующий шаг:** деплой v0.15.0 (⚠️ образ + `postgresql-client-16`, том
`ww_backups`, env) **+ отдать исполнителю хотфикс-стек 0075–0077**; затем
консолидация webapp (TASK-0074) + опц. TASK-0065. Дальше — v1.0 (web-дашборд,

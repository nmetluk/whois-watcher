...
**Обновлено:** 2026-06-06 (✅ TASK-0094: «свободен» НИКОГДА не отдаётся из кэша бота (ADR 046) — повтор stale-free после 0092: фасад отдавал free-запись как «свежую» 6ч без live; теперь free → всегда live (+RDAP-кросс-чек 045). Открыт 🔴 TASK-0095 (инфра, исполнителю): whoisd/relay — негативы ≤60с (дедуп) вместо 1ч, включая ветку RDAP-404; чистка `RU_UPSTREAM_TTL_NEG=3600` из юнитов. Прод ждёт редеплоя 0085-0094; webapp этап 2 «Ещё»-экранов — впереди) · **Релиз на main:** v0.16.0 + hotfix'ы + 0085-0094 (прод ждёт редеплоя) · **Последний ADR:** 046

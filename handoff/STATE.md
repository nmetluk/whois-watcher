...
**Обновлено:** 2026-05-30 (дизайн ADR 039 — багфикс wishlist / v0.11.1) · **Релиз на main:** v0.11.0 · **Последний ADR:** 039
...
| Багфикс wishlist — независимые списки + кнопка удаления | v0.11.1 | 039 ✅ | дизайн готов; исполнение TASK-0031→0032 |
...
✅ **ADR 039 написан** (дизайн — done): багфикс wishlist по двум отзывам.
Корень — `is_wishlist` как флаг на общей строке `user_domains` делает
tracking и wishlist взаимоисключающими. Решение — отдельная таблица
`wishlist` (полная независимость) + кнопка «убрать из wishlist» в `/whois`.
Заведены **TASK-0031** (схема + миграция переноса + drop `is_wishlist`) →
**TASK-0032** (развязка всех вызовов + UX-кнопка + локали). Цепочка
последовательная; рекомендация — вести одним PR (нет промежуточного красного
состояния на public main после `DROP COLUMN`).

**Следующий шаг**: деплой v0.11.0 (`bash scripts/deploy.sh`); по приоритету —
либо **TASK-0031→0032** (срочный багфикс wishlist, негативные отзывы), либо
**TASK-0027** (первый в цепочке v0.12). Wishlist-фикс приоритетнее как
прод-инцидент UX.

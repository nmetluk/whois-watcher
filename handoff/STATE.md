...
исключает юзера; 0033-инварианты сохранены) ✅. **Остался только** 🟢
**TASK-0037** (html.escape + кап интервала FSM). PR #26 **на доработке**
(changes requested, 2026-05-31): код верен и escape покрыт тестом, но не
добавлен FSM-cap тест (инвариант `interval>365`→invalid, граничные 1/365,
366 отклоняется) — мерж заблокирован до него (см. ревью в теле TASK-0037).
После доработки и мержа 0037 → **TASK-0036** «Релиз v0.12.0» (bump
`pyproject` 0.11.1→0.12.0, секция CHANGELOG `[0.12.0]`, тег, деплой).

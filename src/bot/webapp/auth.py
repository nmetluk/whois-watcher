...
Auth middleware for aiohttp: extracts initData from headers, validates,
...
def validate_init_data(
    init_data: str | None, bot_token: str, ttl_seconds: int
) -> WebAppAuthResult:
    ...
    try:
        ...
        raise ValueError(f"bad user payload: {exc}") from None
...
def create_webapp_auth_middleware(settings: Settings) -> web.middleware:
    """Factory: returns aiohttp middleware that does initData auth + user load/register."""

    @web.middleware
    async def _auth_mw(request: web.Request, handler: Any) -> web.StreamResponse:
        ...
        try:
            ...
            return web.json_response(
                {"error": "unauthorized", "reason": str(exc)}, status=401
            )
        except Exception:

...
class Limits(BaseSettings):
    ...
    # ------------------------------------------------------------------
    # Свежесть кэша для синхронных команд (``/whois``, ``/add``)
    # ------------------------------------------------------------------
    whois_cache_fresh_hours: int = Field(
        6,
        ge=1,
        description="Сколько часов считаем кэш WHOIS «свежим» для интерактивных команд.",
    )
    whois_sync_lookup_timeout_seconds: int = Field(
        10,
        ge=1,
        description="Максимум, сколько ждём live-lookup, отвечая на /whois (UX-критично).",
    )
    ...
    # ------------------------------------------------------------------
    # Мониторинг поддоменов (ADR 038)
    # ------------------------------------------------------------------
    max_subdomain_check_interval_days: int = Field(
        365,
        ge=1,
        description="Максимальный интервал проверки поддоменов (дни), который пользователь может задать в FSM (верхний кап).",
    )

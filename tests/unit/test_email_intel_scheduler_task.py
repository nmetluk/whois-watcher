"""Тесты ARQ-задачи ``email_intel_scheduler_tick``.

Параллельно ``test_ssl_scheduler_task`` / ``test_dns_scheduler_task``.
...
class TestBootstrap:
    async def test_bootstrap_sql_executed(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("src.tasks.email_intel_scheduler.get_session", session_cm)
        monkeypatch.setattr(
            "src.tasks.email_intel_scheduler.EmailIntelCacheRepository", lambda _s: cache_repo
        )

        from src.tasks.email_intel_scheduler import email_intel_scheduler_tick

        await email_intel_scheduler_tick(ctx)
...
class TestEnqueueDue:
    async def test_enqueues_each_due_domain(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert len(calls) == 2
        assert calls[0].args == ("check_email_intel", "a.example.com")
        assert calls[1].args == ("check_email_intel", "b.example.com")

...
export const CalendarScreen: React.FC<any> = ({ onOpenDomain, toast }) => {
  ...
  // Ключ heat — ISO YYYY-MM-DD (бэкенд: exp_d.isoformat()); месяц 1-based, с паддингом
  const isoKey = (dd:number) => `${view.y}-${String(view.m+1).padStart(2,'0')}-${String(dd).padStart(2,'0')}`;
  const heat = (dd:number) => { if(!data?.heat) return ''; const n = data.heat[isoKey(dd)]||0; return n===0?'':n<=1?'heat-1':n<=3?'heat-2':'heat-3'; };
